...
namespace FFmpeg.AutoGen.Example
{
    public sealed unsafe class H263VideoStreamEncoder : IDisposable
    {
        ...
        //생성자를 오버로드 
        public H263VideoStreamEncoder(Stream stream, int fps, Size frameSize)
        {
            _stream = stream;
            _frameSize = frameSize;

            var codecId = AVCodecID.AV_CODEC_ID_H263; // 코덱이름
        ...
        public void Encode(AVFrame frame)
        {
            ...
            try
            {
                ...
                using (UnmanagedMemoryStream packetStream = new UnmanagedMemoryStream(pPacket->data, pPacket->size)) packetStream.CopyTo(_stream);
            }
            finally
            {
